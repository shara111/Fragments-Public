...
module.exports.strategy = () =>
  ...
  new BearerStrategy(async (token, done) => {
    try {
      logger.debug({ tokenLength: token.length, tokenStart: token.substring(0, 20) }, 'received token for verification');
      ...
      logger.error({ err, tokenLength: token.length, tokenStart: token.substring(0, 20) }, 'could not verify token');
      done(null, false);
    }
  });
